...
app.get('/api/create-panel', async (req, res) => {
  ...
  try {
    const payUrl = `https://orderkuota-eight.vercel.app/api/orkut/createpayment?apikey=${global.apikeu}&amount=${plan.price}&codeqr=${global.codeqr}`;
...
app.get('/api/create-vps', async (req, res) => {
  ...
  try {
    const payUrl = `https://orderkuota-eight.vercel.app/api/orkut/createpayment?apikey=${global.apikeu}&amount=${vpsPlan.price}&codeqr=${global.codeqr}`;
    const payment = await axios.get(payUrl);
    const payData = payment.data;
    if (!payData.status) return res.status(400).json({ error: 'Gagal membuat pembayaran.' });
